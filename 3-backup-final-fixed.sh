...
get_latest_version() {
	tail -n 1 "$VERSIONS_FILE" | grep -o '"version": *"[^"]*"' | tail -n 1 | cut -d'"' -f4
}
...
for ((i=0; i<$MAX_RUNS; i++)); do
	mkdir -p "$WORK_DIR"
	chmod 777 "$WORK_DIR"
	GIT_SSH_COMMAND="ssh -i $SSH_KEY -o IdentitiesOnly=yes" git -c core.sshCommand="ssh -i $SSH_KEY -o IdentitiesOnly=yes" clone "$REPO_SSH" "$WORK_DIR"
	cd "$WORK_DIR"
	...
	if [[ -n "$MAX_BACKUPS" ]]; then
		mapfile -t backups < <(ls -1t "$BACKUP_DIR"/devops_intership_*.tar.gz 2>/dev/null)
